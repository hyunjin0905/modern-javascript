...
* - 소스 코드 평가 과정에서는 실행컨텍스트를 생성하고 변수 함수 등의 선언문만 먼저 실행하여 생성된 변수나 함수 식별자를 키로 실행 컨텍스트가 관리하는 스코프(렉시컬 환경의 환경 레코드)에 등록한다
* - 소스 코드 평가 과정이 끝나면 비로소 선언문 제외한 소스코드가 순차적으로 실행되기 시작한다 즉 런타임이 시작된다
*   이 때 소스코드 실행에 필요한 정보 즉 변수나 함수의 참조를 실행 컨텍스트가 관리하는 스코프에서 검색해서 취득한다
*   그리고 변수값의 변경 등 소스코드의 실행 결과는 다시 실행 컨텍스트가 관리하는 스코프에 등록된다
*
* */
...
// 1. 전역 코드의 평가
// - 전역 코드 평가 과정 거치며 전역코드 실행 준비
// - 소스코드 평가 과정에서 선언문만 먼저 실행 (변수 선언문 , 함수 선언문)
// - 전역변수 전역 함수가 실행 컨텍스트가 관리하는 전역 스코프에 등록된다
// - var 키워드 로 정의되면 전역 객체의 프로퍼티 , 메서드다
// 2. 전역 코드 실행
// - 전역 코드 평가 과정 끝나면 런타임 시작 되어 전역 코드 순차적으로 실행
// - 전역 변수에 값이 할당되고 함수가 호출 되면 실행 되던 전역코드의 실행을 일시 중단하고 실행 순서를 변경하여 함수 내부로 진입
// 3. 함수 코드 평가
// - 함수 코드 평가 과정을 거치며 함수 코드를 실행하기 준비를 한다
// - 매개 변수, 지역변수문이 먼저 실행 그 결과 생성된 실행 컨텍스트가 관리하는 지역 스코프 등록된다
...
// - console 식별자는 스코프 체인에 등록되어있지 않고 전역 객체에 프로퍼티로 존재한다 이는 전역개체의 프로퍼티가 마치 전역 변수처럼 전역 스코프를 통해 검색 가능해야 한다는 것을 의미
// - log 프로퍼티를 console 객체의 프로토타입 체인을 통해 검색한다  a + x + y  표현식 평가된다  a, x, y 식별자는 스코프 체인을 통해 검색한다 console.log 메서드의 실행이 종료되면  함수 코드 실행과정이 종료
// - 함수 호출 이전 으로 되돌아가 전역 코드 실행을 계속한다
